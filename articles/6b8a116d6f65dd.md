...
画像をランダム表示し、クリックで名前（日本語）、タイプ（日本語）、つよさ（種族値合計）を表示します。子どもからどっちのポケモンが強いのかとよく聞かれるため、つよさは種族値合計値を用いることにしました。GitHub Pagesにデプロイし、PWAとすることでスマホのホーム画面から起動できるようにしました。
